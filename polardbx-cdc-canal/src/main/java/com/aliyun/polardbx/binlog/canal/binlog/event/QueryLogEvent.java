 ...
 * Post-Header has five components:
 * <table>
 ...
 * <td>Error code generated by the master. If the master fails, the slave will fail with the same error code, except for
 * the error codes ER_DB_CREATE_EXISTS == 1007 and ER_DB_DROP_EXISTS == 1008.</td>
 * </tr>
 * <tr>
 * <td>status_vars_len</td>
 * <td>2 byte unsigned integer</td>
 * <td>The length of the status_vars block of the Body, in bytes. See query_log_event_status_vars "below".</td>
 * </tr>
 * </table>
 ...
 * <td>Zero or more status variables. Each status variable consists of one byte identifying the variable stored,
 * followed by the value of the variable. The possible variables are listed separately in the table
 * Table_query_log_event_status_vars "below". MySQL always writes events in the order defined below; however, it is
 * capable of reading them in any order.</td>
 * </tr>
 * <tr>
 * <td>db</td>
 * <td>db_len+1</td>
 * <td>The currently selected database, as a null-terminated string. (The trailing zero is redundant since the length is
 * already known; it is db_len from Post-Header.)</td>
 * </tr>
 * <tr>
 * <td>query</td>
 ...
 * <td>The flags in thd->options, binary AND-ed with OPTIONS_WRITTEN_TO_BIN_LOG. The thd->options bitfield contains
 * options for "SELECT". OPTIONS_WRITTEN identifies those options that need to be written to the binlog (not all do).
 * Specifically, OPTIONS_WRITTEN_TO_BIN_LOG equals (OPTION_AUTO_IS_NULL | OPTION_NO_FOREIGN_KEY_CHECKS |
 * OPTION_RELAXED_UNIQUE_CHECKS | OPTION_NOT_AUTOCOMMIT), or 0x0c084000 in hex. These flags correspond to the SQL
 * variables SQL_AUTO_IS_NULL, FOREIGN_KEY_CHECKS, UNIQUE_CHECKS, and AUTOCOMMIT, documented in the "SET Syntax" section
 * of the MySQL Manual. This field is always written to the binlog in version >= 5.0, and never written in version <
 * 5.0.</td>
 * </tr>
 * <tr>
 * <td>sql_mode</td>
 ...
 * <td>The sql_mode variable. See the section "SQL Modes" in the MySQL manual, and see mysql_priv.h for a list of the
 * possible flags. Currently (2007-10-04), the following flags are available:
 *
 * <pre>
 ...
 * </pre>
 * <p>
 * All these flags are replicated from the server. However, all flags except MODE_NO_DIR_IN_CREATE are honored by the
 * slave; the slave always preserves its old value of MODE_NO_DIR_IN_CREATE. For a rationale, see comment in
 * Query_log_event::do_apply_event in log_event.cc. This field is always written to the binlog.</td>
 * </tr>
 * <tr>
 * <td>catalog</td>
 * <td>Q_CATALOG_NZ_CODE == 6</td>
 * <td>Variable-length string: the length in bytes (1 byte) followed by the characters (at most 255 bytes)</td>
 * <td>Stores the client's current catalog. Every database belongs to a catalog, the same way that every table belongs
 * to a database. Currently, there is only one catalog, "std". This field is written if the length of the catalog is >
 ...
 * <td>The two variables auto_increment_increment and auto_increment_offset, in that order. For more information, see
 * "System variables" in the MySQL manual. This field is written if auto_increment > 1. Otherwise, it is not written.</td>
 * </tr>
 * <tr>
 * <td>charset</td>
 * <td>Q_CHARSET_CODE == 4</td>
 * <td>three 2 byte unsigned integers, totally 2+2+2=6 bytes</td>
 * <td>The three variables character_set_client, collation_connection, and collation_server, in that order.
 * character_set_client is a code identifying the character set and collation used by the client to encode the query.
 * collation_connection identifies the character set and collation that the master converts the query to when it
 * receives it; this is useful when comparing literal strings. collation_server is the default character set and
 * collation used when a new database is created. See also "Connection Character Sets and Collations" in the MySQL 5.1
 * manual. All three variables are codes identifying a (character set, collation) pair. To see which codes map to which
 * pairs, run the query "SELECT id, character_set_name, collation_name FROM COLLATIONS". Cf. Q_CHARSET_DATABASE_CODE
 * below. This field is always written.</td>
 * </tr>
 * <tr>
 * <td>time_zone</td>
 * <td>Q_TIME_ZONE_CODE == 5</td>
 * <td>Variable-length string: the length in bytes (1 byte) followed by the characters (at most 255 bytes).
 * <td>The time_zone of the master. See also "System Variables" and "MySQL Server Time Zone Support" in the MySQL
 * manual. This field is written if the length of the time zone string is > 0; otherwise, it is not written.</td>
 * </tr>
 * <tr>
 * <td>lc_time_names_number</td>
 * <td>Q_LC_TIME_NAMES_CODE == 7</td>
 * <td>2 byte integer</td>
 * <td>A code identifying a table of month and day names. The mapping from codes to languages is defined in
 * sql_locale.cc. This field is written if it is not 0, i.e., if the locale is not en_US.</td>
 * </tr>
 * <tr>
 * <td>charset_database_number</td>
 * <td>Q_CHARSET_DATABASE_CODE == 8</td>
 * <td>2 byte integer</td>
 * <td>The value of the collation_database system variable (in the source code stored in
 * thd->variables.collation_database), which holds the code for a (character set, collation) pair as described above
 * (see Q_CHARSET_CODE). collation_database was used in old versions (???WHEN). Its value was loaded when issuing a
 * "use db" query and could be changed by issuing a "SET collation_database=xxx" query. It used to affect the
 * "LOAD DATA INFILE" and "CREATE TABLE" commands. In newer versions, "CREATE TABLE" has been changed to take the
 * character set from the database of the created table, rather than the character set of the current database. This
 * makes a difference when creating a table in another database than the current one. "LOAD DATA INFILE" has not yet
 * changed to do this, but there are plans to eventually do it, and to make collation_database read-only. This field is
 ...
 * <td>The value of the table map that is to be updated by the multi-table update query statement. Every bit of this
 * variable represents a table, and is set to 1 if the corresponding table is to be updated by this statement. The value
 * of this variable is set when executing a multi-table update statement and used by slave to apply filter rules without
 * opening all the tables on slave. This is required because some tables may not exist on slave because of the filter
 * rules.</td>
 * </tr>
 * </table>
 * Query_log_event_notes_on_previous_versions Notes on Previous Versions Status vars were introduced in version 5.0. To
 * read earlier versions correctly, check the length of the Post-Header. The status variable Q_CATALOG_CODE == 2 existed
 * in MySQL 5.0.x, where 0<=x<=3. It was identical to Q_CATALOG_CODE, except that the string had a trailing '\0'. The
 * '\0' was removed in 5.0.4 since it was redundant (the string length is stored before the string). The Q_CATALOG_CODE
 * will never be written by a new master, but can still be understood by a new slave. See Q_CHARSET_DATABASE_CODE in the
 * table above. When adding new status vars, please don't forget to update the MAX_SIZE_LOG_EVENT_STATUS, and update
 * function code_name
 *
 * @author Changyuan.lh
 * @version 1.0
...
public class QueryLogEvent extends LogEvent {
    ...
    /**
     * The maximum number of updated databases that a status of Query-log-event can carry. It can redefined within a
     * range [1.. OVER_MAX_DBS_IN_EVENT_MTS].
     */
    public static final int MAX_DBS_IN_EVENT_MTS = 16;

    /**
     * When the actual number of databases exceeds MAX_DBS_IN_EVENT_MTS the value of OVER_MAX_DBS_IN_EVENT_MTS is is put
     * into the mts_accessed_dbs status.
     */
    ...
    /**
     * Max number of possible extra bytes in a replication event compared to a packet (i.e. a query) sent from client to
     * master; First, an auxiliary log_event status vars estimation:
     */
     ...
     */
        + 1 + 1 + 255 /*
     * type, length , time_zone
     */
        + 1 + 2 /*
     * type, lc_time_names_number
     */
        + 1 + 2 /*
     * type, charset_database_number
     */
        + 1 + 8 /*
     * type, table_map_for_update
     */
        + 1 + 4 /*
     * type, master_data_written
     */
        /*
         * type, db_1, db_2, ...
         */
     ...
     * 5.0.x where x>=4. Saves one byte in every Query_log_event in binlog, compared
     * to Q_CATALOG_CODE. The reason we didn't simply re-use Q_CATALOG_CODE is that
     * then a 5.0.3 slave of this 5.0.x (x>=4) master would crash (segfault etc)
     * because it would expect a 0 when there is none.
     */
     ...
     * number and their names to be propagated to the slave in order to facilitate
     * the parallel applying of the Query events.
     */
     ...
     * recorded for queries, CREATE and ALTER table that is defined with a TIMESTAMP
     ...
     * rds8.0 no need this variables
     */
    @Deprecated
    public static final int Q_DDL_SKIP_REWRITE = 21;

    /**
     * rds8.0 Replicate opt_flashback_area.
     */
    public static final int Q_OPT_FLASHBACK_AREA = 21;

    /**
     * rds8.0 Replicate opt_index_format_gpp_enabled.
     */
     ...
     * Fixed data part:
     * <ul>
     * <li>4 bytes. The ID of the thread that issued this statement. Needed for temporary tables. This is also useful
     * for a DBA for knowing who did what on the master.</li>
     * <li>4 bytes. The time in seconds that the statement took to execute. Only useful for inspection by the DBA.</li>
     * <li>1 byte. The length of the name of the database which was the default database when the statement was
     * executed. This name appears later, in the variable data part. It is necessary for statements such as INSERT INTO
     * t VALUES(1) that don't specify the database and rely on the default database previously selected by USE.</li>
     * <li>2 bytes. The error code resulting from execution of the statement on the master. Error codes are defined in
     * include/mysqld_error.h. 0 means no error. How come statements with a non-zero error code can exist in the binary
     * log? This is mainly due to the use of non-transactional tables within transactions. For example, if an INSERT ...
     * SELECT fails after inserting 1000 rows into a MyISAM table (for example, with a duplicate-key violation), we have
     * to write this statement to the binary log, because it truly modified the MyISAM table. For transactional tables,
     * there should be no event with a non-zero error code (though it can happen, for example if the connection was
     * interrupted (Control-C)). The slave checks the error code: After executing the statement itself, it compares the
     * error code it got with the error code in the event, and if they are different it stops replicating (unless
     * --slave-skip-errors was used to ignore the error).</li>
     * <li>2 bytes (not present in v1, v3). The length of the status variable block.</li>
     * </ul>
     * Variable part:
     * <ul>
     * <li>Zero or more status variables (not present in v1, v3). Each status variable consists of one byte code
     * identifying the variable stored, followed by the value of the variable. The format of the value is
     * variable-specific, as described later.</li>
     * <li>The default database name (null-terminated).</li>
     * <li>The SQL statement. The slave knows the size of the other fields in the variable part (the sizes are given in
     * the fixed data part), so by subtraction it can know the size of the statement.</li>
     * </ul>
    ...
    private String host;
    /**
     * 'flags2' is a second set of flags (on top of those in Log_event), for session variables. These are thd->options
     * which is & against a mask (OPTIONS_WRITTEN_TO_BIN_LOG).
     */
    private long flags2;
    /**
    ...
    private long prepareGCN = -1;
    private long commitGCN = -1;
     ...
     * <p>
     * The sql_mode variable. See the section "SQL Modes" in the MySQL manual, and see mysql_priv.h for a list of the
     * possible flags. Currently (2007-10-04), the following flags are available:
     * <ul>
     ...
     * </ul>
     * All these flags are replicated from the server. However, all flags except MODE_NO_DIR_IN_CREATE are honored by
     * the slave; the slave always preserves its old value of MODE_NO_DIR_IN_CREATE. This field is always written to the
     * binlog.
     */
    ...
    /**
     * The flags in thd->options, binary AND-ed with OPTIONS_WRITTEN_TO_BIN_LOG. The thd->options bitfield contains
     * options for "SELECT". OPTIONS_WRITTEN identifies those options that need to be written to the binlog (not all
     * do). Specifically, OPTIONS_WRITTEN_TO_BIN_LOG equals (OPTION_AUTO_IS_NULL | OPTION_NO_FOREIGN_KEY_CHECKS |
     * OPTION_RELAXED_UNIQUE_CHECKS | OPTION_NOT_AUTOCOMMIT), or 0x0c084000 in hex. These flags correspond to the SQL
     * variables SQL_AUTO_IS_NULL, FOREIGN_KEY_CHECKS, UNIQUE_CHECKS, and AUTOCOMMIT, documented in the "SET Syntax"
     * section of the MySQL Manual. This field is always written to the binlog in version >= 5.0, and never written in
     * version < 5.0.
     */
    ...
    public long getPrepareGCN() {
        return prepareGCN;
    }

    public void setPrepareGCN(long prepareGCN) {
        this.prepareGCN = prepareGCN;
    }

    public long getCommitGCN() {
        return commitGCN;
    }

    public void setCommitGCN(long commitGCN) {
        this.commitGCN = commitGCN;
    }
}
