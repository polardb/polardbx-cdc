 ...
 * within one version of MySQL. Each event type specifies a format and length of the Post-Header. The length of the
 * Common-Header is the same for all events of the same type. The Body may be of different format and length even for
 * different events of the same type. The binary formats of Post-Header and Body are documented separately in each
 ...
 * <td>total_size</td>
 * <td>4 byte unsigned integer</td>
 * <td>The total size of this event, in bytes. In other words, this is the sum of the sizes of Common-Header,
 * Post-Header, and Body.</td>
 * </tr>
 * <tr>
 * <td>master_position</td>
 * <td>4 byte unsigned integer</td>
 * <td>The position of the next event in the master binary log, in bytes from the beginning of the file. In a binlog
 * that is not a relay log, this is just the position of the next event, in bytes from the beginning of the file. In a
...
public final class LogHeader {
    ...
    /**
     * The offset in the log where this event originally appeared (it is preserved in relay logs, making SHOW SLAVE
     * STATUS able to print coordinates of the event in the master's binlog). Note: when a transaction is written by the
     * master to its binlog (wrapped in BEGIN/COMMIT) the log_pos of all the queries it contains is the one of the BEGIN
    ...
    /**
     * Timestamp on the master(for debugging and replication of NOW()/TIMESTAMP). It is important for queries and LOAD
     * DATA INFILE. This is set at the event's creation time, except for Query and Load (et al.) events where this is
     * set at the query's execution time, which guarantees good replication (otherwise, we could have a query and its
     * event with different timestamps).
     */
    protected long when;
    ...
    /**
     * The master's server id (is preserved in the relay log; used to prevent from infinite loops in circular
     * replication).
     */
    ...
    /**
     * The value is set by caller of FD constructor and Log_event::write_header() for the rest. In the FD case it's
     * propagated into the last byte of post_header_len[] at FD::write(). On the slave side the value is assigned from
     * post_header_len[last] of the last seen FD event.
     */
    ...
    protected FormatDescriptionLogEvent formatDescriptionLogEvent;
    ...
    public LogHeader(LogBuffer buffer, FormatDescriptionLogEvent descriptionEvent) {
        ...
        if ((type == LogEvent.FORMAT_DESCRIPTION_EVENT) || (type == LogEvent.ROTATE_EVENT)) {
            ...
            if (type == LogEvent.FORMAT_DESCRIPTION_EVENT) {
                ...
                String serverVersion =
                    buffer.getFixString(FormatDescriptionLogEvent.ST_SERVER_VER_LEN); // ST_SERVER_VER_OFFSET
        ...
        /*
         * CRC verification by SQL and Show-Binlog-Events master side. The caller has to provide
         * @description_event->checksum_alg to be the last seen FD's (A) descriptor. If event is FD the descriptor is in
         * it. Notice, FD of the binlog can be only in one instance and therefore Show-Binlog-Events executing master
         * side thread needs just to know the only FD's (A) value - whereas RL can contain more. In the RL case, the alg
         * is kept in FD_e (@description_event) which is reset to the newer read-out event after its execution with
         * possibly new alg descriptor. Therefore in a typical sequence of RL: {FD_s^0, FD_m, E_m^1} E_m^1 will be
         * verified with (A) of FD_m. See legends definition on MYSQL_BIN_LOG::relay_log_checksum_alg docs lines
         * (log.h). Notice, a pre-checksum FD version forces alg := BINLOG_CHECKSUM_ALG_UNDEF.
         */
...
//        processCheckSum(buffer);
    ...
    public void initDataBuffer(LogBuffer buffer) {
        dataBuffer = buffer.getData(startPos, eventLen);
    }
     ...
     * a relay log, this is the position of the next event in the master's binlog.
     */
    ...
    /**
     * The total size of this event, in bytes. In other words, this is the sum of the sizes of Common-Header,
    ...
    public FormatDescriptionLogEvent getFormatDescriptionLogEvent() {
