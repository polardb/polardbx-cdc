...
public final class FormatDescriptionLogEvent extends StartLogEventV3 {
    ...
    public FormatDescriptionLogEvent(LogHeader header, LogBuffer buffer, FormatDescriptionLogEvent descriptionEvent)
        ...
        long newPos = buffer.position();
        int len = (int) (newPos - header.getStartPos());
        bufferData = new byte[len];
        buffer.position((int) header.getStartPos());
        buffer.fillBytes(bufferData, 0, len);
    }
    ...
    @Override
    public String info() {
        return String.format("Server ver: %s, Binlog ver: %s", serverVersion, binlogVersion);
    }
}
