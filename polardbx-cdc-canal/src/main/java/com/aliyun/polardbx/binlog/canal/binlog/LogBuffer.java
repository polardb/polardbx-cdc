...
public class LogBuffer {
    ...
    private static final int getInt32BE(byte[] buffer, final int pos) {
        return (buffer[pos] << 24) | ((0xff & buffer[pos + 1]) << 16) | ((0xff & buffer[pos + 2]) << 8) | (0xff
            & buffer[pos
            + 3]);
    }
    ...
    /**
     * Sets this buffer's position. If the mark is defined and larger than the new position then it is discarded. </p>
     *
     ...
     * mark is defined and larger than the new limit then it is discarded. </p>
     *
    ...
    /**
     * Tells whether there are any elements between the current position and the limit. </p>
     *
    ...
    /**
     * Return packed number from buffer. (little-endian) A Packed Integer has the capacity of storing up to 8-byte
     * integers, while small integers still can use 1, 3, or 4 bytes. The value of the first byte determines how to read
     * the number, according to the following table.
     * <ul>
     * <li>0-250 The first byte is the number (in the range 0-250). No additional bytes are used.</li>
     * <li>252 Two more bytes are used. The number is in the range 251-0xffff.</li>
     * <li>253 Three more bytes are used. The number is in the range 0xffff-0xffffff.</li>
     * <li>254 Eight more bytes are used. The number is in the range 0xffffff-0xffffffffffffffff.</li>
     * </ul>
    ...
    /**
     * Return fix-length string from buffer without null-terminate checking. Fix bug #17 {@link https
    ...
    /**
     * Return next fix-length string from buffer without null-terminate checking. Fix bug #17 {@link https
     * ://github.com/AlibabaTech/canal/issues/17 }
     */
