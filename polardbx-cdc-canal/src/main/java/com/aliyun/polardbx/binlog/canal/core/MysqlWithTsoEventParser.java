 ...
 * @since 3.2.5
 */
public class MysqlWithTsoEventParser extends AbstractMysqlEventParser implements BinlogEventParser {

    private static final Logger logger = LoggerFactory.getLogger(MysqlWithTsoEventParser.class);
    private MysqlConnection metaConnection;                                                             // 查询meta信息的链接

    public int fallbackIntervalInSeconds = 60;                                             // 切换回退时间
    ...
    public int dumpErrorCount = 0;                                              // binlogDump失败异常计数
    public int dumpTimeoutCount = 0;
    public int dumpErrorCountThreshold = 2;                                              // binlogDump失败异常计数阀值

    public MysqlWithTsoEventParser() {
    ...
    private class TailLogEventFilter implements LogEventFilter {
        ...
        public void handle(HandlerEvent event, HandlerContext context) throws Exception {
            dumpTimeoutCount = 0;
            dumpErrorCount = 0;
            MysqlWithTsoEventParser.this.eventHandler.handle(event);
        }
