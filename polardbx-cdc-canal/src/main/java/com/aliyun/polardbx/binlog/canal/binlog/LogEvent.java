...
/**
 * Binary log event definitions. This includes generic code common to all types of log events, as well as specific code
 * for each type of log event. - All numbers, whether they are 16-, 24-, 32-, or 64-bit numbers, are stored in little
 * endian, i.e., the least significant byte first, unless otherwise specified. representation of unsigned integers,
 * called Packed Integer. A Packed Integer has the capacity of storing up to 8-byte integers, while small integers still
 * can use 1, 3, or 4 bytes. The value of the first byte determines how to read the number, according to the following
 * table:
 * <table>
 ...
 * <tr>
 * <td>0-250</td>
 * <td>The first byte is the number (in the range 0-250), and no more bytes are used.</td>
 * </tr>
 * <tr>
 * <td>252</td>
 ...
 * <tr>
 * <td>254</td>
 * <td>Eight more bytes are used. The number is in the range 0xffffff-0xffffffffffffffff.</td>
 * </tr>
 * </table>
 ...
 */
public abstract class LogEvent implements HandlerEvent {
     ...
     * the counter is overflowed and restarts from 0 ? - Query and Load (Create or Execute) events may have a more
     * precise timestamp (with microseconds), number of matched/affected/warnings rows and fields of session variables:
     * SQL_MODE, FOREIGN_KEY_CHECKS, UNIQUE_CHECKS, SQL_AUTO_IS_NULL, the collations and charsets, the PASSWORD()
     * version (old/new/...).
     */
    ...
    /**
     * NEW_LOAD_EVENT is like LOAD_EVENT except that it has a longer sql_ex, allowing multibyte TERMINATED BY etc; both
     * types share the same class (Load_log_event)
     */
    ...
    /*
     * Binlog checkpoint event. Used for XA crash recovery on the master, not used in replication. A binlog checkpoint
     * event specifies a binlog file such that XA crash recovery can start from that file - and it is guaranteed to find
     * all XIDs that are prepared in storage engines but not yet committed.
     */
    public static final int BINLOG_CHECKPOINT_EVENT = 161;
    /*
     * Gtid event. For global transaction ID, used to start a new event group, instead of the old BEGIN query event, and
     * also to mark stand-alone events.
     */
    public static final int GTID_EVENT = 162;
    /*
     * Gtid list event. Logged at the start of every binlog, to record the current replication state. This consists of
     * the last GTID seen for each replication domain.
     */
    ...
    /**
     * 1 byte length, 1 byte format Length is total length in bytes, including 2 byte header Length values 0 and 1 are
     * currently invalid and reserved.
     */
    ...
    /*
     * MariaDB >= 5.5. This version has the capability to tolerate events omitted from the binlog stream without
     * breaking replication (MySQL slaves fail because they mis-compute the offsets into the master's binlog).
     */
    ...
    /**
     * For an event, 'e', carrying a type code, that a slave, 's', does not recognize, 's' will check 'e' for
     * LOG_EVENT_IGNORABLE_F, and if the flag is set, then 'e' is ignored. Otherwise, 's' acknowledges that it has found
     * an unknown event in the relay log.
     */
    ...
    public final FormatDescriptionLogEvent getDescriptionEvent() {
        return this.header.getFormatDescriptionLogEvent();
    }
    ...
    public String info() {
        return getTypeName(getHeader().getType());
    }
