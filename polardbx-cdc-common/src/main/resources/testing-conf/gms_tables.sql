...
(
    ...
    `need_apply`     tinyint(1) default 1 not null,
    PRIMARY KEY (`id`),
...
(
    ...
    `ddl_stmt` longtext,
    `async_flag` tinyint(1) NOT NULL DEFAULT '0',
    `async_state` varchar(32) NOT NULL,
    `schema_name` varchar(100) DEFAULT NULL,
    `table_name` varchar(100) DEFAULT NULL,
    `task_id` bigint(20) unsigned NOT NULL DEFAULT '0',
    `parallel_seq` int(11) NOT NULL DEFAULT '0',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_fsm_ddl_tso` (`fsm_id`,`ddl_tso`),
    UNIQUE KEY `uk_token` (`token`),
    KEY `service_id` (`service_id`),
    KEY `idx_async` (`async_flag`,`async_state`),
    KEY `idx_schema_table` (`schema_name`,`table_name`),
    KEY `idx_fsm_task` (`fsm_id`,`task_id`)
) ENGINE=InnoDB AUTO_INCREMENT=57730 DEFAULT CHARSET=utf8
/*!40101 SET character_set_client = @saved_cs_client */;
...
(
   ...
   `parallel_seq` int(11) NOT NULL DEFAULT '0',
   PRIMARY KEY (`id`),
   UNIQUE KEY `uk_fsm_task_tso` (`fsm_id`,`task_id`,`ddl_tso`),
   KEY `fsm_service_tso` (`fsm_id`,`service_id`,`ddl_tso`)
...
(
    ...
    `type`                varchar(32)  NOT NULL COMMENT '全量校验, 增量校验',
    `state`               varchar(32)  NOT NULL COMMENT '新创建, 复检等等',
...
(
    ...
    `type`                varchar(32) NOT NULL COMMENT '校验任务类型. 1 : 全量校验, 2 : 增量校验',
    `state`               varchar(32) NOT NULL COMMENT '当前任务运行状态 ',
...
(
    20
) unsigned not null comment '状态机id',
    src_logical_db varchar
(
    128
) comment '源端逻辑库名',
    src_logical_table varchar
(
    128
) comment '源端逻辑表名',
    dst_logical_db varchar
(
    128
) comment '目标端逻辑库名',
    dst_logical_table varchar
(
    128
) comment '目标端表名',
    task_stage varchar
(
    128
) not null comment '任务阶段',
    task_state varchar
(
    128
) not null comment '任务状态, RUNNING/PAUSED/FINISHED',
    task_config longtext null comment '任务配置信息, json string',
...
(
    ...
    full_valid_task_id bigint
(
    20
) unsigned not null comment '全量校验任务id',
    task_type varchar
(
    128
...
(
    128
) not null comment '任务状态, READY/RUNNING/PAUSED/FINISHED',
    task_config longtext null comment '任务配置信息, json string',
    error_info longtext null comment '任务执行出错信息',
    create_time datetime default CURRENT_TIMESTAMP not null,
    update_time datetime default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP,
    primary key
(
...
(
    128
) null comment '目标端逻辑库名',
    dst_logical_table varchar
(
    128
) null comment '目标端表名',
    src_key_col_val varchar
(
    256
) null comment '源端 pk/uk值',
    dst_key_col_val varchar
(
    256
) null comment '目标端 pk/uk值',
    error_type varchar
(
    128
) not null comment '错误类型, DIFF/MISS/ORPHAN',
    status varchar
(
    128
) not null comment '状态, FOUND/REPAIRED',
    create_time datetime default CURRENT_TIMESTAMP not null,
    update_time datetime default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP,
    primary key
(
