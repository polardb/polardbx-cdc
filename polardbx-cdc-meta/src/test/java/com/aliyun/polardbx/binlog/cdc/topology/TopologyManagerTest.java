...
public class TopologyManagerTest {
    ...
    public void applyAndCheck() {
        ...
        manager.apply("d1", null, r1);//create db
        Assert.assertEquals("d1", manager.getLogicSchema("d1_000001").getSchema());
        manager.apply("d1", "t1", r2);//create table d1.t1
        ...
        manager.apply("d1", null, r3);//move group
        ...
        manager.apply("d1", "t2", r4);//create table d1.t2
        ...
        manager.apply("d1", null, r5);//move group back
        tables = manager.getPhyTables("polardbx-storage-0-master").stream()
            .flatMap(p -> p.getPhyTables().stream()).collect(
                Collectors.toList());
