...
public class StorageCountStrategyTest extends BaseTest {

    @Test
    public void apply() {
        mockConfig(ConfigKeys.CLUSTER_TOPOLOGY_EXCLUDE_NODES, "[]");
        List<NodeInfo> nodeInfoList = TableDataMocker.mockNodeInfoList();
        try (MockedStatic<SpringContextHolder> holder = mockStatic(SpringContextHolder.class, CALLS_REAL_METHODS)) {
            ...
            when(nodeInfoService.getAliveNodes(any())).thenReturn(nodeInfoList);
            when(SpringContextHolder.getObject(NodeInfoService.class)).thenReturn(nodeInfoService);
